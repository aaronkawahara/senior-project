...
def convert(cvt, path, subpath):
    ...
    for _, _, filesnames in os.walk(path):
        for file in filesnames:
            ...
            width = c_uint16(width)
            height= c_uint16(height)
            v = [cvt(r, g, b) for (r, g, b) in im.getdata()]
            b = bytearray()
            b.append(width.value & 0xff)
            b.append((width.value & 0xff00) >> 8)
            b.append(height.value & 0xff)
            b.append((height.value & 0xff00) >> 8)
            for pair in v:
                for i in pair:
                    b.append(i)
