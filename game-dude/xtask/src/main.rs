// fill with tasks like makefile
// functions correspond to tasks
// ex) fn test_all, fn test_host, fn test_host_target
...
fn flash() -> Result<(), anyhow::Error> {
    let _p = xshell::pushd(root_dir().join("cross"))?;
    cmd!("cargo flash --chip STM32L4P5ZGTx --release").run()?;
    Ok(())
}
